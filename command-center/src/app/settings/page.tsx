...
import { BASE_PAY_TYPES, BasePayType, KpiTargets, MANAGED_DROPDOWNS, PayRules, ServiceCatalogItem, TechBasePayRule } from "@/lib/types";
...
function ServiceCatalog() {
  ...
  const [fActive, setFActive] = useState("All");   // All | Active | Inactive
  const [fReport, setFReport] = useState("All");   // All | Reporting | Excluded
  const [draft, setDraft] = useState<ServiceCatalogItem | null>(null);
  if (!can("manageSettings")) return null;

  const categories = Array.from(new Set(s.serviceCatalog.map((c) => c.category))).sort();
  const rows = s.serviceCatalog.filter((c) => {
    if (q && !c.serviceName.toLowerCase().includes(q.toLowerCase())) return false;
    if (fCat !== "All" && c.category !== fCat) return false;
    if (fActive === "Active" && !c.active) return false;
    if (fActive === "Inactive" && c.active) return false;
    if (fReport === "Reporting" && !c.activeForJobReporting) return false;
    if (fReport === "Excluded" && c.activeForJobReporting) return false;
    return true;
  }).sort((a, b) => (a.category + a.serviceName).localeCompare(b.category + b.serviceName));

  const saveDraft = () => { if (draft) { s.updateServiceItem(draft); setDraft(null); } };

  return (
    <Card className="p-4">
      {s.serviceCatalog.length === 0 ? (
        <div className="text-sm text-muted">No services yet. Use <b className="text-ink">Seed service catalog (CSV)</b> in the GHL Webhook &amp; Cloud panel to load them.</div>
      ) : (
        <>
          <div className="flex flex-wrap gap-2 mb-3">
            <Input placeholder="Search service name…" value={q} onChange={(e) => setQ(e.target.value)} className="w-64" />
            <Select options={["All", ...categories]} value={fCat} onChange={(e) => setFCat(e.target.value)} className="w-auto" />
            <Select options={["All", "Active", "Inactive"]} value={fActive} onChange={(e) => setFActive(e.target.value)} className="w-auto" />
            <Select options={["All", "Reporting", "Excluded"]} value={fReport} onChange={(e) => setFReport(e.target.value)} className="w-auto" />
            <span className="text-xs text-muted self-center">{rows.length} of {s.serviceCatalog.length} services</span>
          </div>
          <div className="border border-line rounded-lg divide-y divide-line/50 max-h-[28rem] overflow-y-auto">
            {rows.map((c) => (
              <div key={c.id} className="flex items-center gap-2 px-3 py-2">
                <div className="min-w-0 flex-1">
                  <div className="text-sm text-ink truncate">{c.serviceName}</div>
                  <div className="text-xs text-muted truncate">{c.category}{c.price ? ` · ${c.price}` : ""}</div>
                </div>
                {c.isAddon && <span className="text-[10px] uppercase tracking-wide text-muted">add-on</span>}
                <StatusPill label={c.active ? "Active" : "Inactive"} tone={c.active ? "good" : "neutral"} />
                <StatusPill label={c.activeForJobReporting ? "Reporting" : "Excluded"} tone={c.activeForJobReporting ? "info" : "warn"} />
                <Button variant="ghost" onClick={() => setDraft({ ...c })}>Edit</Button>
                <Button variant="ghost" onClick={() => s.updateServiceItem({ ...c, active: !c.active })}>{c.active ? "Deactivate" : "Activate"}</Button>
              </div>
            ))}
            {!rows.length && <div className="text-sm text-muted px-3 py-4">No services match these filters.</div>}
          </div>
          <p className="text-xs text-muted mt-3">Owner/Admin only. Services are never deleted — <b className="text-ink">deactivate</b> instead (records that already use a service are unchanged). &quot;Excluded&quot; means the service stays in the catalog but is left out of job reporting (e.g. Gift Cards, 5280 Care Club).</p>
        </>
      )}

      <Modal open={!!draft} onClose={() => setDraft(null)} title="Edit Service">
        {draft && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="sm:col-span-2"><Field label="Service Name"><Input value={draft.serviceName} onChange={(e) => setDraft({ ...draft, serviceName: e.target.value })} /></Field></div>
            <Field label="Category"><Select options={categories.includes(draft.category) ? categories : [draft.category, ...categories]} value={draft.category} onChange={(e) => setDraft({ ...draft, category: e.target.value })} /></Field>
            <Field label="Price (reference)"><Input value={draft.price} onChange={(e) => setDraft({ ...draft, price: e.target.value })} placeholder="e.g. $100 or Coupe/Sedan: $100, SUV: $110" /></Field>
            <Field label="Active"><Select options={["Yes", "No"]} value={draft.active ? "Yes" : "No"} onChange={(e) => setDraft({ ...draft, active: e.target.value === "Yes" })} /></Field>
            <Field label="Active for Job Reporting"><Select options={["Yes", "No"]} value={draft.activeForJobReporting ? "Yes" : "No"} onChange={(e) => setDraft({ ...draft, activeForJobReporting: e.target.value === "Yes" })} /></Field>
            <div className="sm:col-span-2"><Field label="Description"><Textarea rows={4} value={draft.description} onChange={(e) => setDraft({ ...draft, description: e.target.value })} /></Field></div>
          </div>
        )}
        <div className="flex justify-end gap-2 mt-4">
          <Button onClick={() => setDraft(null)}>Cancel</Button>
          <Button variant="accent" onClick={saveDraft}>Save Service</Button>
        </div>
      </Modal>
    </Card>
  );
...
function KpiTargetsEditor() {
  const s = useStore();
  const { can } = useAuth();
  const [d, setD] = useState<KpiTargets>(s.kpiTargets);
  const [saved, setSaved] = useState(false);
  useEffect(() => { setD(s.kpiTargets); }, [s.kpiTargets]);
  if (!can("manageSettings")) return null;
  const f = (key: keyof KpiTargets, label: string) => (
    <Field label={label}><Input type="number" value={d[key]} onChange={(e) => { setD({ ...d, [key]: +e.target.value }); setSaved(false); }} /></Field>
  );
  return (
    <Card className="p-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {f("soloDailyRevenue", "Solo daily revenue $")}{f("soloDailyJobsMin", "Solo daily jobs (min)")}{f("soloDailyJobsMax", "Solo daily jobs (max)")}
        {f("soloDaysPerWeek", "Solo days/week")}{f("duoDailyRevenue", "Duo daily revenue $")}{f("duoDailyJobsMin", "Duo daily jobs (min)")}
        {f("duoDailyJobsMax", "Duo daily jobs (max)")}{f("duoDaysPerWeek", "Duo days/week")}{f("mobileWeeklyRevenue", "Mobile weekly revenue $")}
        {f("ceramicDailyRevenue", "Ceramic daily revenue $")}{f("ceramicDailyJobsMin", "Ceramic daily jobs (min)")}{f("ceramicDailyJobsMax", "Ceramic daily jobs (max)")}
        {f("shopDailyRevenue", "Shop daily revenue $")}{f("mobileTintDailyRevenue", "Mobile tint daily revenue $")}{f("mobileTintDaysPerWeek", "Mobile tint days/week")}
        {f("inShopTintDailyRevenue", "In-shop tint daily revenue $")}{f("inShopTintDailyJobs", "In-shop tint daily jobs")}{f("inShopDetailsDailyRevenue", "In-shop details daily revenue $")}
        {f("monthlyGoalNoTint", "Monthly goal — no tint $")}{f("monthlyGoalWithTint", "Monthly goal — with tint $")}{f("futureMonthlyGoal", "Future monthly goal $")}
      </div>
      <div className="flex items-center gap-3 mt-3">
        <Button variant="accent" onClick={() => { s.setKpiTargets(d); setSaved(true); }}>Save KPI Targets</Button>
        {saved && <span className="text-sm text-good">✓ Saved.</span>}
      </div>
      <p className="text-xs text-muted mt-2">Owner/Admin only. Defaults for the Overview KPI Targets — editable, not hardcoded. Daily figures are prorated by the date range on Overview.</p>
    </Card>
  );
}
...
export default function SettingsPage() {
  ...
  return (
    <div>
      ...
      <Section title="Service Catalog"><ServiceCatalog /></Section>

      <Section title="KPI Targets"><KpiTargetsEditor /></Section>

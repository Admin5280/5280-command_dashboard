...
export interface CareAdvanced {
  churnRate: number; cancellationRate: number; avgDurationDays: number; avgDurationMonths: number;
  avgVisitsPerMember: number; totalMemberVisits: number;
}
/** Churn/cancellation/duration/visit metrics. `from`/`to` scope the churn window. */
export function careAdvancedMetrics(members: CareMember[], visits: CareVisit[], from: string, to: string): CareAdvanced {
  const active = members.filter((m) => m.memberStatus === "Active");
  const totalMemberVisits = visits.filter((v) => v.visitStatus === "Completed").length;
  const canceledAll = members.filter((m) => m.memberStatus === "Canceled").length;
  const canceledInPeriod = members.filter((m) => m.memberStatus === "Canceled" && inRange(m.cancelDate, from, to)).length;
  const activeAtStart = active.length + canceledInPeriod;   // approximation: active-now + those who left in the window
  const durs = active.map(memberDurationDays).filter((d) => d > 0);
  const avgDurationDays = durs.length ? durs.reduce((a, b) => a + b, 0) / durs.length : 0;
  return {
    churnRate: activeAtStart ? canceledInPeriod / activeAtStart : 0,
    cancellationRate: members.length ? canceledAll / members.length : 0,
    avgDurationDays, avgDurationMonths: avgDurationDays / 30.44,
    avgVisitsPerMember: active.length ? totalMemberVisits / active.length : 0,
    totalMemberVisits,
  };
}
/** Completed visits tied to a member. */
export const memberLifetimeVisits = (memberId: string, visits: CareVisit[]) =>
  visits.filter((v) => v.memberId === memberId && v.visitStatus === "Completed").length;
/** Lifetime value = cash paid + add-on revenue + tips across the member's visits. */
export const memberLifetimeValue = (m: CareMember, visits: CareVisit[]) =>
  (m.amountPaid || 0) + sum(visits.filter((v) => v.memberId === m.id), (v) => (v.addOnRevenue || 0) + (v.tip || 0));
/** Days since the member started (start date, else signup date). */
export const memberDurationDays = (m: CareMember): number => {
  const start = m.startDate || m.signupDate;
  if (!start) return 0;
  return Math.max(0, Math.round((new Date(today() + "T00:00:00").getTime() - new Date(start + "T00:00:00").getTime()) / 86400000));
};

...
export interface Lead {
  ...
  // v2 Sales close + deposit tracking (optional so existing lead literals stay valid)
  closedDate?: string;
  closedBySalesRep?: string;
  ...
  depositCollectedDate?: string;
  depositStatus?: DepositStatus;
  proposalSentDate?: string;
  quoteSentDate?: string;
}

/** A job add-on (from service_catalog where category = "Add ons"); tracked separately from the main service. */
export interface JobAddOn {
  serviceId: string;   // service_catalog.id
  name: string;
  price: string;       // reference price from the catalog (may hold size tiers)
}
...
export interface Job {
  ...
  createdAt: string;
  updatedAt: string;
  // v2 service-catalog linkage (optional so existing job literals stay valid)
  mainServiceId?: string;      // service_catalog.id of the main service
  serviceCategory?: string;    // auto-filled from the main service's catalog category (drives reporting)
  addOns?: JobAddOn[];         // selected add-ons; never overwrite serviceCategory
  depositApplied?: number;     // deposit carried from the lead; part of amountPaid (never double-counted)
}

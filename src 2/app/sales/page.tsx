...
import { Lead, Job, CareClubLead, LEAD_STATUSES, LEAD_SOURCES, SOURCE_REVIEW_STATUSES, CLAIM_STATUSES, DEPOSIT_STATUSES } from "@/lib/types";
import { money, num, pct, prettyDate, safeDiv, groupBy, sum, today } from "@/lib/format";
import { byGroup, isConverted, salesCloseKpis } from "@/lib/metrics";
...
/* ================= Sales Close & Deposit dashboard ================= */
function SalesCloseDashboard() {
  const s = useStore();
  const todayStr = today();
  const weekStartStr = new Date(new Date(today() + "T00:00:00").getTime() - 6 * 86400000).toISOString().slice(0, 10);
  const kpis = useMemo(() => salesCloseKpis(s.leads, todayStr, weekStartStr), [s.leads, todayStr, weekStartStr]);

  const [fRep, setFRep] = useState("All");
  const [fDep, setFDep] = useState("All");
  const [q, setQ] = useState("");
  const text = (l: Lead) => `${l.leadId} ${l.customerName} ${l.phone} ${l.email} ${l.depositReference ?? ""}`.toLowerCase();

  const rows = useMemo(() => s.leads
    .filter((l) => l.closedDate)
    .filter((l) => fRep === "All" || l.closedBySalesRep === fRep || l.assignedSalesRep === fRep)
    .filter((l) => fDep === "All" || l.depositStatus === fDep)
    .filter((l) => !q || text(l).includes(q.toLowerCase()))
    .sort((a, b) => (b.closedDate ?? "").localeCompare(a.closedDate ?? "")),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [s.leads, fRep, fDep, q]);

  const cols: Col<Lead>[] = [
    { key: "closedDate", label: "Closed Date", render: (l) => <span className="text-muted">{prettyDate(l.closedDate ?? "")}</span> },
    ...
    { key: "assignedSalesRep", label: "Assigned Rep", render: (l) => l.assignedSalesRep || "—" },
    { key: "closedBySalesRep", label: "Closed By", render: (l) => l.closedBySalesRep || "—" },
    { key: "bookedJobValue", label: "Booked Job Value", render: (l) => <span className="tabular-nums text-gold">{money(l.bookedJobValue)}</span> },
    { key: "depositRequired", label: "Deposit Req.", render: (l) => (l.depositRequired ? "Yes" : "No") },
    { key: "depositAmount", label: "Deposit Amount", render: (l) => <span className="tabular-nums">{money(l.depositAmount || 0)}</span> },
    { key: "depositStatus", label: "Deposit Status", render: (l) => (l.depositStatus ? <Badge value={l.depositStatus} /> : <span className="text-muted">—</span>) },
    { key: "bookingConfirmedDate", label: "Booking Confirmed", render: (l) => <span className="text-muted">{prettyDate(l.bookingConfirmedDate ?? "") || "—"}</span> },
    { key: "ghl", label: "GHL", render: (l) => <LinkOut href={l.ghlContactLink} /> },
  ];

  return (
    <>
      <Section title="Sales Close Dashboard">
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-3">
          <Kpi label="Leads Closed Today" value={num(kpis.leadsClosedToday)} tone="blue" />
          <Kpi label="Leads Closed This Week" value={num(kpis.leadsClosedWeek)} tone="blue" />
          <Kpi label="Confirmed Bookings Today" value={num(kpis.confirmedBookingsToday)} tone="blue" />
          <Kpi label="Confirmed Bookings This Week" value={num(kpis.confirmedBookingsWeek)} tone="blue" />
          <Kpi label="Deposit Collected Today" value={money(kpis.depositCollectedToday)} tone="gold" />
          <Kpi label="Deposit Collected This Week" value={money(kpis.depositCollectedWeek)} tone="gold" />
          <Kpi label="Closed Revenue Today" value={money(kpis.closedRevenueToday)} tone="gold" />
          <Kpi label="Closed Revenue This Week" value={money(kpis.closedRevenueWeek)} tone="gold" />
          <Kpi label="Average Closed Deal" value={money(kpis.avgClosedDeal)} tone="gold" />
          <Kpi label="Deposit Collection Rate" value={pct(kpis.depositCollectionRate)} tone="blue" />
        </div>
      </Section>

      <Section title={`Daily Closed Leads (${rows.length})`}>
        <div className="flex flex-wrap gap-2 mb-3">
          <Select options={["All", ...s.salesReps]} value={fRep} onChange={(e) => setFRep(e.target.value)} className="w-auto" />
          <Select options={["All", ...DEPOSIT_STATUSES]} value={fDep} onChange={(e) => setFDep(e.target.value)} className="w-auto" />
          <Input placeholder="Search lead / customer / phone / email / deposit ref…" value={q} onChange={(e) => setQ(e.target.value)} className="w-72" />
        </div>
        <Table cols={cols} rows={rows} empty="No closed leads yet." />
      </Section>
    </>
  );
}
